...
void runMatrixExperiment(unsigned int min, unsigned int max, unsigned int repeats, const std::string &file_out) {
  std::cout << "Matrix inner product benchmark." << std::endl;
  // Attempt to open the file for writing
  std::ofstream fos(file_out);
  if (!fos.good()) {
    std::cerr << "Could not open file " + file_out << std::endl;
...
void runMatrixExperimentOMP(unsigned int min,
  ...
  std::cout << "Matrix inner product benchmark OMP." << std::endl;
  // Attempt to open the file for writing
  std::ofstream fos(file_out);
  if (!fos.good()) {
    std::cerr << "Could not open file " + file_out << std::endl;
  ...
  // Dump a header
  generateHeader(ss,
                 {"Experiment", "Matrix size", "Threads", "Construct (s)", "Randomize (s)"},
                 {"Float", "Double"},
                 repeats);
  dump(ss, fos, std::cout);
...
void runMatrixExperimentOCL(unsigned int min, unsigned int max, unsigned int repeats, const std::string &file_out) {
  ...
  std::cout << "Matrix inner product benchmark OCL." << std::endl;
  // Attempt to open the file for writing
  std::ofstream fos(file_out);
  if (!fos.good()) {
    std::cerr << "Could not open file " + file_out << std::endl;
  ...
  for (unsigned int e = min; e < max; e++) {
    ...
    //for (unsigned int r = 0; r < repeats; r++) {
      t.start();  // Start the timer.
      // Multiply the matrices
      auto mat_result = multiplyMatricesOCL(mat_a, mat_b);
      t.stop();  // Stop the timer.
      t.report(ss);
    ...
    //for (unsigned int r = 0; r < repeats; r++) {
      t.start();
      auto mat_result2 = multiplyMatricesOCL(mat_c, mat_d);
      t.stop();
      t.report(ss);
      dump(ss, fos, std::cout);
      std::cout << "\n";
